...
# ETAPA 5:
# Vamos alterar todos os .faa para .fasta
rename 's/.faa$/.fasta/' *.faa

# ETAPA 6:
# Agora vamos fazer a quebra de linha padrão para arquivos FASTA, onde cada linha da SEQUENCIA possui 60 caracteres.
# Loop para processar cada arquivo .fasta
for file in *.fasta; do
    # Cria um arquivo temporário
    output="${file%.fasta}_formatted.fasta"
    
    # Processa o arquivo, quebrando as sequências a cada 60 caracteres
    awk '/^>/ {print (NR==1 ? "" : "\n") $0; next} { printf "%s", $0 } END { print "" }' "$file" |
    fold -w 60 > "$output"

    echo "Arquivo formatado gerado: $output"
done

# ETAPA 7:
# Agora vamos limpar e organizar os arquivos novamente.
rename 's/_formatted.fasta$/.faa/' *_formatted.fasta
rm *.fasta
rename 's/.faa$/.fasta/' *.faa
